  ...
  return withTransaction(async (client) => {
    ...
    const reportResult = await client.query(
      `INSERT INTO interview_reports
       ...
       RETURNING id, interview_id, interviewer_id, candidate_id,
                 interviewer_name, candidate_name, interviewer_email, candidate_email,
                 question_category, room_id, duration, report_type, created_at`,
      [
    ...
    const savedRow = reportResult.rows[0];
    const reportId = savedRow.id;

    // ── Compute content hash from the exact values Postgres stored ────────────
    const contentHash = computeContentHash(savedRow);
    ...
    // Use the created_at that Postgres actually stored (from RETURNING) — not the JS string
    // This ensures save-time and verify-time use the exact same value
    const blockCreatedAt = savedRow.created_at;
