...
export function generateInterviewReportPDF(reportData) {
    ...
    if (evaluation.questionWiseResults && evaluation.questionWiseResults.length > 0) {
        ...
        evaluation.questionWiseResults.forEach((item, index) => {
            ...
            doc.setFillColor(...colors.light);
            doc.rect(10, yPosition, pageWidth - 20, 24, 'F');

            doc.setTextColor(...colors.dark);
            doc.setFontSize(10);
            doc.setFont('helvetica', 'bold');
            doc.text(`Q${index + 1}: ${item.questionText || 'Question'}`, 15, yPosition + 6, { maxWidth: pageWidth - 55 });

            doc.setTextColor(...scoreColor);
            doc.setFontSize(11);
            doc.text(`${qScore}%`, pageWidth - 20, yPosition + 6, { align: 'right' });

            doc.setTextColor(...colors.dark);
            doc.setFontSize(9);
            doc.setFont('helvetica', 'normal');
            const answerPreview = (item.answer || '').slice(0, 150) + ((item.answer || '').length > 150 ? '...' : '');
            const previewLines = doc.splitTextToSize(`Answer: ${answerPreview}`, pageWidth - 30);
            doc.text(previewLines.slice(0, 2), 15, yPosition + 13);
