...
export function generateInterviewReportPDF(reportData) {
    ...
    // Metrics summary (real values from question-wise results)
    doc.setTextColor(...colors.dark);
    doc.setFontSize(14);
    doc.setFont('helvetica', 'bold');
    doc.text('Summary Metrics', 15, yPosition);
    
    yPosition += 8;

    const sm = evaluation.summaryMetrics || {};
    const metrics = [
        { name: 'Alignment avg', score: sm.alignmentAverage ?? evaluation.overallScore ?? 0, suffix: '%' },
        { name: 'Voice<->Typed avg', score: sm.voiceTypedAverage ?? evaluation.typedAiDetection?.averageVoiceTypedMatch ?? 0, suffix: '%' },
        { name: 'Typed AI avg conf', score: sm.typedAiAverageConfidence ?? evaluation.typedAiDetection?.typedAverageConfidence ?? 0, suffix: '%' },
        { name: 'Paste flagged', score: sm.pasteSuspectedCount ?? 0, suffix: '' }
    ];
    ...
    metrics.forEach((metric, index) => {
        ...
        doc.setFont('helvetica', 'normal');
        doc.text(`${Number(metric.score) || 0}${metric.suffix}`, pageWidth / 2, yPosition + 8, { align: 'center' });
        
        // status label only makes sense for percentage-like metrics
        if (metric.suffix === '%') {
            const status = getScoreStatus(Number(metric.score) || 0);
            const statusColor = (Number(metric.score) || 0) >= 80 ? colors.success : (Number(metric.score) || 0) >= 60 ? colors.warning : colors.danger;
