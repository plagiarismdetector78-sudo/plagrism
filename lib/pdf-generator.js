    ...
    if (evaluation.questionWiseResults && evaluation.questionWiseResults.length > 0) {
        ...
        if (evaluation.typedAiDetection) {
            ...
            const summary = [
                ...
                `Typed AI-flagged: ${t.typedAiGeneratedCount || 0}`,
        ...
        evaluation.questionWiseResults.forEach((item, index) => {
            ...
            const typedScoreLine = Array.isArray(typedAi?.allScores)
                ? typedAi.allScores.map((s) => `${s.label}: ${Math.round((s.score || 0) * 100)}%`).join(' | ')
                : '';
            const typedScoreLines = typedScoreLine ? doc.splitTextToSize(`Scores: ${typedScoreLine}`, pageWidth - 30) : [];
            ...
            doc.setFontSize(11);
            doc.setFont('helvetica', 'bold');
            doc.text(`${qScore}%`, pageWidth - 20, titleStartY + lineH * 0.85, { align: 'right' });
