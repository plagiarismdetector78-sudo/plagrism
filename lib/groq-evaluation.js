...
/** Best token overlap of spoken answer vs reference expected answers only (not the question text). */
function spokenExpectedHeuristicPercent(spokenAnswer, expectedAnswers) {
  const spoken = String(spokenAnswer || "").trim();
  if (!spoken || !(expectedAnswers || []).length) return 0;
  let best = 0;
...
/**
 * If the candidate mostly repeats the question (high overlap with question, low with expected answers),
 * do not treat that as a strong match to the reference answer.
 */
function questionParrotAlignmentCap(spokenAnswer, questionText, expectedAnswers) {
  const spoken = String(spokenAnswer || "").trim();
  const qt = String(questionText || "").trim();
  if (!spoken || !qt || !(expectedAnswers || []).length) return null;

  const jq = jaccardPercentStrings(spoken, qt);
  let bestExp = 0;
  for (const exp of expectedAnswers) {
    bestExp = Math.max(bestExp, jaccardPercentStrings(spoken, exp));
  }

  if (jq >= 82 && bestExp < 28) {
    return { cap: 20, jq, bestExp };
  }
  if (jq >= 70 && bestExp < 22) {
    return { cap: 15, jq, bestExp };
  }
  return null;
}
...
export async function analyzeQuestionWithGroq({
  ...
  const system = `You are a strict, expert technical interview evaluator with deep knowledge of computer science and software engineering.
Your job is to score a candidate's spoken answer against the reference expected answer for a given interview question.
Be precise and consistent. Use the full 0-100 range — do not cluster scores around 50.
Return ONLY valid JSON. No explanation outside the JSON.`;

  const user = `You are evaluating a technical interview answer. Think step by step before scoring.

## Question
${payload.questionText}

## Reference Expected Answer(s)
${payload.expectedAnswers.length > 0 ? payload.expectedAnswers.map((a, i) => `[${i + 1}] ${a}`).join("\n") : "(none provided)"}

## Candidate's Spoken Answer
${payload.spokenAnswer || "(empty — candidate did not speak)"}

## Candidate's Typed Answer
${payload.typedAnswer || "(empty)"}

## Voice-Typed Similarity (pre-computed token overlap %)
${payload.voiceTypedSimilarity ?? "N/A"}

## Typing Metadata
${payload.typedMeta ? JSON.stringify(payload.typedMeta) : "N/A"}

---

## Scoring Instructions

### spoken_expected_alignment.score (0–100)
Think: Does the spoken answer demonstrate understanding of the concept in the expected answer?
- 0   → spoken answer is empty OR completely off-topic (no relevant concept at all)
- 10–25 → mentions the topic but shows no real understanding
- 26–45 → partially correct, missing most key points
- 46–65 → covers some key points correctly, missing others
- 66–80 → covers most key points with minor gaps or imprecision
- 81–100 → comprehensive, accurate, covers all key points

### voice_typed_consistency.score (0–100)
- Use the pre-computed voice-typed similarity as the BASE.
- If either spoken or typed is empty → 0.
- Adjust ±10 based on semantic alignment between spoken and typed.

### typed_authorship_risk
- Analyze paste patterns, typing speed, and whether the typed answer reads like AI-generated text.
- "high" only if there is strong evidence: large paste, AI-style phrasing, no typing metadata.
- "low" if typed naturally with normal keystroke patterns.

Now return ONLY this JSON (no markdown, no extra text):
{
  ...
  "typed_authorship_risk": { "level": "low"|"medium"|"high", "confidence": <0-100>, "reasons": [<string>], "paste_suspected": <bool> },
  ...
  if (
    ...
    const retryUser =
      user +
      `\n\nIMPORTANT: You returned score=0 for spoken_expected_alignment. Re-read the spoken answer carefully. If it contains ANY correct technical concept related to the question or expected answer, the score MUST be greater than 0. A partial answer with one correct point should score at least 20.\n`;
    try {
  ...
  const spokenHeuristic = spokenExpectedHeuristicPercent(payload.spokenAnswer, payload.expectedAnswers);
  const groqAlignmentScore = clamp100(alignment.score);
  let alignmentScore = groqAlignmentScore;
  let alignmentParrotCapped = false;
  ...
  const parrotCap = questionParrotAlignmentCap(
    payload.spokenAnswer,
    payload.questionText,
    payload.expectedAnswers
  );
  if (parrotCap && alignmentScore > parrotCap.cap) {
    console.log("[GroqEval] spoken_expected_alignment capped (question parrot)", {
      before: alignmentScore,
      cap: parrotCap.cap,
      jaccardVsQuestion: parrotCap.jq,
      bestJaccardVsExpected: parrotCap.bestExp,
    });
    alignmentScore = parrotCap.cap;
    alignmentParrotCapped = true;
  }
  ...
  let alignmentReasons = Array.isArray(alignment.reasons)
    ...
    : [];
  if (alignmentParrotCapped) {
    alignmentReasons = [
      "Alignment capped: the spoken answer largely repeats the question and does not substantively match the reference expected answer.",
      ...alignmentReasons,
    ].slice(0, 12);
  }
  if (alignmentScore > groqAlignmentScore && spokenHeuristic > 0) {
    alignmentReasons = [
      `Token overlap vs reference expected answer (floor): ~${spokenHeuristic}%.`,
      ...alignmentReasons,
    ].slice(0, 12);
  }
  ...
  return {
    ...
    typed_authorship_risk: {
      level: (() => {
        // Groq's risk often conflates "bad/short answer" with "AI-written".
        // We treat *very short* answers (with no paste signal) as "insufficient evidence",
        // so they should not become high AI risk.
        const level = ["low", "medium", "high"].includes(String(risk.level)) ? String(risk.level) : "low";
        const typed = String(payload.typedAnswer || "").trim();
        const pasteCount = Number(payload?.typedMeta?.pasteCount || 0);
        const pastedChars = Number(payload?.typedMeta?.pastedChars || 0);
        if (typed.length > 0 && typed.length < 40 && pasteCount === 0 && pastedChars === 0) {
          return "low";
        }
        return level;
      })(),
      confidence: (() => {
        const conf = clamp100(risk.confidence);
        const typed = String(payload.typedAnswer || "").trim();
        const pasteCount = Number(payload?.typedMeta?.pasteCount || 0);
        const pastedChars = Number(payload?.typedMeta?.pastedChars || 0);
        if (typed.length > 0 && typed.length < 40 && pasteCount === 0 && pastedChars === 0) {
          return Math.min(conf, 40);
        }
        return conf;
      })(),
      reasons: (() => {
        const reasons = Array.isArray(risk.reasons) ? risk.reasons.map(String).slice(0, 12) : [];
        const typed = String(payload.typedAnswer || "").trim();
        const pasteCount = Number(payload?.typedMeta?.pasteCount || 0);
        const pastedChars = Number(payload?.typedMeta?.pastedChars || 0);
        if (typed.length > 0 && typed.length < 40 && pasteCount === 0 && pastedChars === 0) {
          return [
            "AI risk downgraded: typed answer is too short to reliably infer AI-writing patterns (no paste signal).",
            ...reasons,
          ].slice(0, 12);
        }
        return reasons;
