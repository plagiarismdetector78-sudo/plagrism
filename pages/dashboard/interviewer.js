      ...
      >
        ...
        {showCandidatesModal && !isMobile && (
          <div className="fixed inset-0 z-[70] flex items-start justify-center p-4"
               style={{ paddingTop: '80px', paddingLeft: sidebarCollapsed ? '96px' : '304px' }}>
            {/* Backdrop */}
            <div
            ...
            <div
              ...
              className="relative backdrop-blur-xl bg-gray-800/95 rounded-3xl border border-white/10 shadow-2xl w-full max-w-4xl flex flex-col"
              style={{ maxHeight: 'calc(100vh - 96px)' }}
              onClick={(e) => e.stopPropagation()}
            >
             {/* Header Row (icon, title, search, close btn all aligned) */}
<div className="flex justify-between items-center mb-4 w-full px-6 pt-6">
  ...
  <button
    onClick={() => setShowCandidatesModal(false)}
    className="text-gray-400 hover:text-white p-2 rounded-xl hover:bg-white/10 transition flex-shrink-0"
...
<div className="flex items-center space-x-3 px-6 pb-4">
  <input
    ...
    className="flex-1 px-4 py-2.5 bg-white/10 border border-white/20 rounded-xl text-white placeholder-gray-400 focus:ring-2 focus:ring-purple-500 focus:outline-none"
  />
  <button
    onClick={() => {
      if (searchQuery.trim() !== "") {
    ...
    className="flex-shrink-0 bg-gradient-to-r from-purple-500 to-pink-500 px-5 py-2.5 rounded-xl text-white font-medium hover:scale-105 transition"
  >
...
<div className="flex-1 overflow-y-auto px-6 pb-6" style={{ minHeight: 0 }}>
  ...
  {(() => {
    ...
    if (candidates.length === 0) {
      return (
        <p className="text-gray-400 text-center py-10">No candidates registered yet.</p>
      );
    }
    ...
    return filtered.map((candidate) => (
      <div
        key={candidate.id}
        className="bg-white/5 border border-white/10 rounded-2xl p-4 flex justify-between items-center hover:border-purple-500/30 transition-all duration-300"
      >
        {/* Left: Info */}
        <div className="flex items-center space-x-3 min-w-0">
          ...
          <div className="min-w-0">
            ...
            {candidate.phone && candidate.phone !== "N/A" && (
              <p className="text-gray-400 text-xs mt-0.5">{candidate.phone}</p>
            )}
            {candidate.qualification && (
              <p className="text-gray-400 text-xs mt-0.5 truncate">{candidate.qualification}</p>
            )}
            {candidate.matchScore && (
              <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium border mt-1 ${candidate.compatibilityColor}`}>
                Match: {candidate.matchScore}/10
              </span>
            )}
          </div>
        </div>
        ...
        <button
          ...
          className="flex-shrink-0 ml-4 bg-gradient-to-r from-purple-500 to-pink-500 hover:from-purple-600 hover:to-pink-600 text-white px-4 py-2 rounded-xl text-sm font-medium transition-all duration-300 hover:scale-105"
        >
          Schedule
        </button>
      </div>
